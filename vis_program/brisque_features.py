...
def lmom(X):
    ...
    b0 = X.mean()
    for r in range(1, 4):
        Num = np.prod(np.tile(np.arange(r + 1, n + 1), (r, 1)) - np.tile(np.arange(1, r + 1).reshape(r, 1), (1, n - r)),
                      0)
...
def compute_features(im):
    ...
    window = get_gaussian_filter(shape = 3, sigma = 2.0)
    scalenum = 2
    feat = []
...
im = ndimage.imread('1.bmp', flatten=True)
